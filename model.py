...
def encode_prompt(text, vocab, add_bos=True):
    ...
    # we already have a vocab, which consists of
    # token -> id and id -> token

    # we check if bos is in token -> id
    tokens = []
    if ("<bos>" in vocab["token_to_id"] and add_bos == True):
        tokens.append(vocab["token_to_id"]["<bos>"])
    text_characters = list(text)
    for char in text_characters:
        if char in vocab["token_to_id"]:
            tokens.append(vocab["token_to_id"][char])
        else:
...
def decode_tokens(token_ids, vocab, skip_special=True):
    ...
    for token_id in token_ids:
        if "<" in vocab["id_to_token"][token_id]:
...
def append_kv(cache, k_new, v_new):
    ...
    current_cache_length = cache['length']
    
    #append to respective K and V
    for k, v in zip(k_new, v_new):
        cache['K'][current_cache_length] = k
        cache['V'][current_cache_length] = v
        current_cache_length += 1

    cache['length'] += k_new.shape[0]
    return cache
